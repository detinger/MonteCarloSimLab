      ...
      <div className="grid" style={{ gridTemplateColumns: '1fr 2.5fr', gap: '1.5rem', alignItems: 'stretch' }}>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <Card title="Parameters">
            <Input label={<span>Initial Price (<Latex math="S_0" />)</span>} value={s0} onChange={setS0} min={1} />
            <Input label={<span>Expected Return (<Latex math="\mu" />)</span>} value={mu} onChange={setMu} step={0.01} />
            <Input label={<span>Volatility (<Latex math="\sigma" />)</span>} value={sigma} onChange={setSigma} step={0.01} min={0.01} />
        ...
        <div>
          <Card title="Simulated Paths">
            ...
            <p className="text-muted" style={{ fontSize: '0.85rem', marginTop: '1rem' }}>
              Using the Geometric Brownian Motion model: <Latex math="dS_t = \mu S_t dt + \sigma S_t dW_t" block={true} />
            </p>
          </Card>
        </div>
      </div>
